...
{
	int r;
	usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "");

#ifdef _MSC_VER
	if(ctx) {
		ctx = NULL;	/* assume old context has been destructed */
	}
#endif
	if (!ctx) {
		r = libusb_init(&ctx);
		if (r < 0) {
			usbi_log(LOG_LEVEL_ERROR, __FUNCTION__, "initialization failed!");
			return;
		}
...
static int find_busses(struct usb_bus **ret)
{
	...
	if (r < 0) {
		usbi_log(LOG_LEVEL_ERROR, __FUNCTION__, "get_device_list failed with error %d", r);
		return compat_err(r);
	}
...
API_EXPORTED int usb_find_busses(void)
{
	...
	if (r < 0) {
		usbi_log(LOG_LEVEL_ERROR, __FUNCTION__, "find_busses failed with error %d", r);
		return r;
	}
	...
	while (bus) {
		...
		int found = 0;
		usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "in loop", 0);
		...
		if (!found) {
			/* bus removed */
			usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "bus %d removed", bus->location);
			changes++;
	...
	while (bus) {
		struct usb_bus *tbus = bus->next;
		usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "bus %d added", bus->location);
...
static int initialize_device(struct usb_device *dev)
{
	...
	if (r < 0) {
		usbi_log(LOG_LEVEL_ERROR, __FUNCTION__, "error %d getting device descriptor", r);
		return compat_err(r);
	}
...
API_EXPORTED int usb_find_devices(void)
{
	...
	for (bus = usb_busses; bus; bus = bus->next) {
		...
		while (dev) {
			...
			if (!found) {
				usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "device %d.%d removed",
		...
		while (dev) {
			...
			if (r < 0) {
				usbi_log(LOG_LEVEL_ERROR, __FUNCTION__, "couldn't initialize device %d.%d (error %d)",
					dev->bus->location, dev->devnum, r);
				dev = tdev;
				continue;
			}
			usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "device %d.%d added", dev->bus->location, dev->devnum);
...
API_EXPORTED usb_dev_handle *usb_open(struct usb_device *dev)
{
	...
	if (r < 0) {
		usbi_log(LOG_LEVEL_ERROR, __FUNCTION__, "could not open device, error %d", r);
		free(udev);
...
{
	usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "configuration %d", configuration);
...
{
	int r;
	usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "interface %d", interface);
...
{
	usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "alternate %d", alternate);
...
{
	usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "endpoint %x", ep);
	...
	int size, int timeout)
{
	int actual_length;
	int r;
	usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "endpoint %x size %d timeout %d", ep, size, timeout);
	...
	int size, int timeout)
{
	if (!(ep & USB_ENDPOINT_IN)) {
		...
		usbi_log(LOG_LEVEL_WARNING, __FUNCTION__, "endpoint %x is missing IN direction bit, fixing");
		ep |= USB_ENDPOINT_IN;
	}
	...
	int size, int timeout)
{
	if (ep & USB_ENDPOINT_IN) {
		 ...
		 * warn about this silly behaviour. */
		usbi_log(LOG_LEVEL_WARNING, __FUNCTION__, "endpoint %x has excessive IN direction bit, fixing");
		ep &= ~USB_ENDPOINT_IN;
	}
...
{
	int r;
	usbi_log(LOG_LEVEL_DEBUG, __FUNCTION__, "RQT=%x RQ=%x V=%x I=%x len=%d timeout=%d", bmRequestType,
...
// libusb-win32 specific functions
API_EXPORTED int usb_bulk_setup_async(usb_dev_handle *dev, void **context, unsigned char ep){ assert(0); return 0; };
API_EXPORTED int usb_cancel_async(void *context){ assert(0); return 0; };
API_EXPORTED int usb_free_async(void **context){ assert(0); return 0; };
API_EXPORTED const struct usb_version *usb_get_version(void){ assert(0); return 0; };
API_EXPORTED int usb_install_driver_np(const char *inf_file){ assert(0); return 0; };
API_EXPORTED void CALLBACK usb_install_driver_np_rundll(HWND wnd, HINSTANCE instance, LPSTR cmd_line, int cmd_show){ assert(0); return; };
API_EXPORTED int usb_install_needs_restart_np(void){ assert(0); return 0; };
API_EXPORTED int usb_install_service_np(void){ assert(0); return 0; };
API_EXPORTED void CALLBACK usb_install_service_np_rundll(HWND wnd, HINSTANCE instance, LPSTR cmd_line, int cmd_show){ assert(0); return; };
API_EXPORTED int usb_interrupt_setup_async(usb_dev_handle *dev, void **context, unsigned char ep){ assert(0); return 0; };
API_EXPORTED int usb_isochronous_setup_async(usb_dev_handle *dev, void **context, unsigned char ep, int pktsize){ assert(0); return 0; };
API_EXPORTED int usb_reap_async(void *context, int timeout){ assert(0); return 0; };
API_EXPORTED int usb_reap_async_nocancel(void *context, int timeout){ assert(0); return 0; };
API_EXPORTED int usb_submit_async(void *context, char *bytes, int size){ assert(0); return 0; };
API_EXPORTED int usb_touch_inf_file_np(const char *inf_file){ assert(0); return 0; };
API_EXPORTED void CALLBACK usb_touch_inf_file_np_rundll(HWND wnd, HINSTANCE instance, LPSTR cmd_line, int cmd_show){ assert(0); return; };
API_EXPORTED int usb_uninstall_service_np(void){ assert(0); return 0; };
API_EXPORTED void CALLBACK usb_uninstall_service_np_rundll(HWND wnd, HINSTANCE instance, LPSTR cmd_line, int cmd_show){ assert(0); return; };
